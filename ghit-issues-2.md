...
---
type: Feature
title: "Replace any[] placeholders with shared domain types in a central types module"
labels: type:refactor, area:types, stack:nextjs, stack:react, stack:typescript, priority:medium, MAYBE REWARDED, GRANTFOX OSS, OFFICIAL CAMPAIGN
assignees: ''
---

## Refactor scattered any[] placeholders into shared domain types

### Description
Three pages declare untyped placeholders — `const contracts: any[] = []` in [`src/app/contracts/page.tsx`](src/app/contracts/page.tsx), `const milestones: any[] = []` in [`src/app/milestones/page.tsx`](src/app/milestones/page.tsx), and `const reputation: any[] = []` in [`src/app/reputation/page.tsx`](src/app/reputation/page.tsx) — while the real shapes already exist as `Milestone` ([`MilestonesList`](src/components/MilestonesList.tsx)), the contract props ([`ContractSummary`](src/components/ContractSummary.tsx)), and `ReputationProfileProps` ([`ReputationProfile`](src/components/ReputationProfile.tsx)). The `any[]` defeats type safety. This issue consolidates the domain types and removes the `any`s.

### Requirements and context
- **Repository scope:** Talenttrust/Talenttrust-Frontend only.
- Create `src/types/domain.ts` re-exporting/defining the canonical `Contract`, `Milestone`, and reputation types, sourced from the existing component exports to avoid drift.
- Replace each `any[]` placeholder with the proper typed empty array (`const contracts: Contract[] = []`, etc.) so the empty/list branches remain type-checked.
- Do not change runtime behavior or the existing `EmptyState` rendering; this is a typing-only refactor on the placeholders.
- Ensure `StatusType` and currency fields stay consistent across modules.

### Suggested execution
- Fork the repo and create a branch
- `git checkout -b refactor/types-34-shared-domain-types`
- Implement changes
  - **Write code in:** create [`src/types/domain.ts`](src/types/domain.ts); update [`src/app/contracts/page.tsx`](src/app/contracts/page.tsx), [`src/app/milestones/page.tsx`](src/app/milestones/page.tsx), [`src/app/reputation/page.tsx`](src/app/reputation/page.tsx).
  - **Write comprehensive tests in:** ensure the existing page tests stay green; add a type-level test or render assertion proving the typed empty state still shows `EmptyState`.
  - **Add documentation:** add a "Domain types" note to [`README.md`](README.md) or `docs/`.
  - Add JSDoc to the exported types.
  - Validate that `tsc`/`npm run build` passes with no `any`.
- Test and commit

### Test and commit
- Run `npm run lint`, `npm test`, and `npm run build`.
- Cover edge cases: empty arrays still render EmptyState, types compile, no behavior change.
- Include the full `npm test` output and notes in the PR description.

### Example commit message
`refactor(types): replace any[] placeholders with shared domain types`
